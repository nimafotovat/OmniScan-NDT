...
# ۲. تولید سیگنال ایده‌آل (عیب در عمق ۲۰ میلی‌متری)
defect_depth_m = 0.02
probe_freq = 5e6
...
# ۳. اعمال نویز گاوسی (SNR = 20 dB که نویز نسبتاً مشهودی است)
noisy_echo = add_gaussian_noise(clean_echo, snr_db=20.0)

# ۴. اعمال فیلتر میان‌گذر (حدود پهنای باند پروب: از ۲ تا ۸ مگاهرتز)
filtered_echo = apply_bandpass_filter(
    signal=noisy_echo, lowcut=2e6, highcut=8e6, sample_rate=SAMPLE_RATE, order=4
)

# ۵. تبدیل محور زمان به محور عمق (میلی‌متر) برای نمایش صنعتی A-Scan
depth_array_mm = (time_array * STEEL_VELOCITY / 2) * 1000

# ۶. رسم نمودار حرفه‌ای و مقایسه‌ای
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 7), sharex=True)

# نمودار بالا: سیگنال نویزی
...
# نمودار پایین: سیگنال فیلتر شده (خروجی نهایی دستگاه)
